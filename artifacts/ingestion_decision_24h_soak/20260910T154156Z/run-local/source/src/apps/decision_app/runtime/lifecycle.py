...
async def capture_lifecycle_tail(
    stream_client: Any,
    *,
...
class LifecycleNotificationReader:
    ...
    def __init__(
        ...
        *,
        ...
        if io_timeout_seconds is not None and block_ms / 1000 >= float(
            io_timeout_seconds
        ):
            raise ValueError("block_ms must be shorter than io_timeout_seconds")
        if (
        ...
        try:
            raw = await run_with_timeout(
                self._client.xread(
                    ...
                    block=None if self._block_ms == 0 else self._block_ms,
                ),
                self._io_timeout_seconds,
                operation="lifecycle XREAD",
            )
