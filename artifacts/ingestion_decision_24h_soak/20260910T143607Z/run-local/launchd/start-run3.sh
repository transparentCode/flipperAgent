...
RUN3_ID="20260910T143607Z"
RUN3_DIR="$RUN3_ROOT/artifacts/ingestion_decision_24h_soak/$RUN3_ID"
RUN3_PROJECT="flipper-id-preflight-20260910t143607z"
...
validate_startup_health() {
  "$RUN3_PYTHON" -c '
import sys
expected = {"db", "broker", "ingestion", "decision", "otel-collector", "prometheus", "grafana"}
rows = [line.strip().split("|") for line in sys.stdin if line.strip()]
if len(rows) != 7 or any(len(row) != 4 for row in rows):
    raise SystemExit(1)
if {row[0] for row in rows} != expected:
    raise SystemExit(1)
if any(state != "running" or oom != "false" or health not in {"healthy", "starting"} for _, state, health, oom in rows):
    raise SystemExit(1)
raise SystemExit(75 if any(row[2] == "starting" for row in rows) else 0)
'
}

wait_for_run_health() {
  local remaining ids result snapshot
  local -a containers
  while true; do
    remaining="$(run3_remaining_seconds)" || return 1
    ids="$(docker ps -aq --filter "label=com.docker.compose.project=$RUN3_PROJECT")" || return 1
    containers=("${(@f)ids}")
    (( ${#containers} == 7 )) || return 1
    result=0
    snapshot="$(docker inspect --format '{{index .Config.Labels "com.docker.compose.service"}}|{{.State.Status}}|{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}|{{.State.OOMKilled}}' \
      "${containers[@]}")" || return 1
    print -r -- "$snapshot" | validate_startup_health || result=$?
    if (( result == 0 )); then
      run3_remaining_seconds >/dev/null || return 1
      return 0
    fi
    (( result == 75 )) || return 1
    remaining="$(run3_remaining_seconds)" || return 1
...
# Compose creation is not readiness. Only normal health=starting may wait;
# every other topology/state defect fails before the guarded child execs sampler.
wait_for_run_health
