...
SAMPLER_PATH = Path(__file__).with_name("legacy_sampler.py")
SAMPLER_SPEC = importlib.util.spec_from_file_location(
    "run3_legacy_sampler", SAMPLER_PATH
)
if SAMPLER_SPEC is None or SAMPLER_SPEC.loader is None:
    raise RuntimeError("cannot load legacy sampler")
SAMPLER = importlib.util.module_from_spec(SAMPLER_SPEC)
SAMPLER_SPEC.loader.exec_module(SAMPLER)


def _boundary_soak() -> object:
    soak = HARNESS.Soak.__new__(HARNESS.Soak)
    soak.history_cursors = {}
    soak.boundary_episodes = {}
    soak.completed_episodes = []
    soak.correctness_failure = False
    soak.event = Mock()
    return soak


class Run3HarnessSemanticsTests(unittest.TestCase):
    def test_host_environment_does_not_invoke_power_probe(self):
        soak = SAMPLER.Soak.__new__(SAMPLER.Soak)
        soak.run_dir = Path("/tmp/hostguard-test-run")
        calls = []
        ...
        soak.command = command

        snapshot = soak.host_environment()

        self.assertEqual(
            [argv for argv, _timeout in calls],
            [("vm_stat",), ("df", "-k", "/tmp/hostguard-test-run")],
        )
        self.assertEqual(set(snapshot), {"memory_pressure", "disk", "load_average"})
        self.assertNotIn("power", snapshot)
    ...
    def test_expected_boundary_starts_only_advanced_series_with_stale_history(self):
        soak = _boundary_soak()
        btc_1h = "BTC-USDT-PERP:1h"
        btc_4h = "BTC-USDT-PERP:4h"
        eth_4h = "ETH-USDT-PERP:4h"
        old = "2026-08-24T14:00:00+00:00"
        expected = "2026-08-24T15:00:00+00:00"
        soak.history_cursors = {
            btc_1h: old,
            btc_4h: old,
            eth_4h: old,
            "SOL-USDT-PERP:1h": old,
        }
        history = {
            "required_series": {
                btc_1h: {"last_close": old, "expected_closed_boundary": expected},
                btc_4h: {"last_close": old, "expected_closed_boundary": old},
                eth_4h: {"last_close": old, "expected_closed_boundary": old},
                "SOL-USDT-PERP:1h": {
                    "last_close": old,
                    "expected_closed_boundary": expected,
                },
            }
        }
        input_lags = {key: 0 for key in HARNESS.REQUIRED_INPUTS}
        input_lags["stream:ohlcv:ingestion:binance:BTC-USDT-PERP:1h"] = 1
        result = soak._advance_boundary_episodes(
            history=history,
            input_lags=input_lags,
            lane_lags_by_series={btc_1h: 1, btc_4h: 0, eth_4h: 0},
            provenance={btc_1h: {"source": "test"}},
            blocked=0,
            probe_valid=True,
            now=datetime(2026, 8, 24, 15, 0, tzinfo=UTC),
        )

        self.assertEqual(result["pending"], [btc_1h])
        self.assertEqual(set(soak.boundary_episodes), {btc_1h})
        self.assertEqual(soak.boundary_episodes[btc_1h]["boundary_at"], expected)
        self.assertEqual(
            soak.boundary_episodes[btc_1h]["provenance"], {"source": "test"}
        )

    def test_stale_canonical_history_can_converge_within_window(self):
        soak = _boundary_soak()
        series = "BTC-USDT-PERP:1h"
        input_key = "stream:ohlcv:ingestion:binance:BTC-USDT-PERP:1h"
        old = "2026-08-24T14:00:00+00:00"
        expected = "2026-08-24T15:00:00+00:00"
        history = {
            "required_series": {
                series: {"last_close": old, "expected_closed_boundary": expected}
            }
        }
        soak.history_cursors = {series: old}
        soak._advance_boundary_episodes(
            history=history,
            input_lags={input_key: 1},
            lane_lags_by_series={series: 1},
            provenance={series: {"canonical_history_current_at": old}},
            blocked=0,
            probe_valid=True,
            now=datetime(2026, 8, 24, 15, 0, tzinfo=UTC),
        )
        result = soak._advance_boundary_episodes(
            history=history,
            input_lags={input_key: 0},
            lane_lags_by_series={series: 0},
            provenance={series: {"canonical_history_current_at": old}},
            blocked=0,
            probe_valid=True,
            now=datetime(2026, 8, 24, 15, 0, 20, tzinfo=UTC),
        )

        self.assertEqual(result, {"pending": [], "unresolved": []})
        self.assertEqual(len(soak.completed_episodes), 1)
        self.assertEqual(
            soak.completed_episodes[0]["status"], HARNESS.TRANSIENT_CONVERGED
        )
        self.assertFalse(soak.correctness_failure)

    def test_indeterminate_probe_does_not_become_lag_failure(self):
        soak = _boundary_soak()
        series = "BTC-USDT-PERP:1h"
        old = "2026-08-24T14:00:00+00:00"
        expected = "2026-08-24T15:00:00+00:00"
        ...
        result = soak._advance_boundary_episodes(
            ...
            input_lags={"stream:ohlcv:ingestion:binance:BTC-USDT-PERP:1h": None},
            lane_lags_by_series={},
            provenance={},
            blocked=0,
            probe_valid=False,
        ...
        self.assertEqual(result, {"pending": [series], "unresolved": []})
        self.assertFalse(soak.correctness_failure)
        self.assertIsNone(soak.boundary_episodes[series]["first_valid_observation_at"])

    def test_expired_unresolved_episode_terminates_measurement_loop(self):
        soak = _boundary_soak()
        series = "BTC-USDT-PERP:1h"
        input_key = "stream:ohlcv:ingestion:binance:BTC-USDT-PERP:1h"
        boundary = datetime(2026, 8, 24, 15, 0, tzinfo=UTC)
        soak.history_cursors = {series: boundary.isoformat()}
        soak.boundary_episodes = {
            series: {
                "series": series,
                "boundary_at": boundary.isoformat(),
                "first_valid_observation_at": boundary.isoformat(),
                "deadline_at": (boundary + timedelta(seconds=120)).isoformat(),
        ...
        expiry = boundary + timedelta(seconds=121)

        def expired_sample(**_kwargs):
            soak._advance_boundary_episodes(
                history={
                    "required_series": {
                        series: {
                            "last_close": boundary.isoformat(),
                            "expected_closed_boundary": boundary.isoformat(),
                        }
                    }
                },
                input_lags={input_key: 1},
        ...
        soak.hard_failure = False
        soak.measurement_completed = False
        soak.measurement_started_monotonic = None
        soak.args = SimpleNamespace(measurement_seconds=86400)
        soak.begin_measurement = Mock(
            side_effect=lambda: setattr(soak, "measurement_started_monotonic", 0.0)
        )
        soak.sample = Mock(side_effect=expired_sample)
        soak.measurement_elapsed = Mock(return_value=121.0)
        soak._verify_frozen_tooling = Mock(return_value=True)
        soak._write_state = Mock()

        with patch.object(HARNESS.time, "monotonic", side_effect=[0.0, 0.0, 1.0]):
            soak.measure()

        self.assertEqual(soak.sample.call_count, 2)
        self.assertEqual(soak.completed_episodes[0]["status"], HARNESS.UNRESOLVED)
        self.assertTrue(soak.correctness_failure)
        self.assertFalse(soak.measurement_completed)
