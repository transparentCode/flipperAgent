...
def boundary_advanced(previous: datetime | None, current: datetime | None) -> bool:
    """Return true only when a tracked closed boundary moved forward."""
    ...
    def _advance_boundary_episodes(
        ...
        *,
        ...
        for series, row in rows.items() if isinstance(rows, dict) else ():
            if series not in REQUIRED_SERIES or not isinstance(row, dict):
                continue
            expected_boundary = _parse_timestamp(row.get("expected_closed_boundary"))
            if expected_boundary is None:
                continue
            previous = _parse_timestamp(self.history_cursors.get(series))
            if previous is None:
                self.history_cursors[series] = expected_boundary.isoformat()
                continue
            if boundary_advanced(previous, expected_boundary):
                self.history_cursors[series] = expected_boundary.isoformat()
                ...
                if probe_valid and input_lag == 0 and watermark_lag == 0:
                    self.event(
                        ...
                        series=series,
                        boundary_at=expected_boundary.isoformat(),
                    )
                else:
                    episode = {
                        "series": series,
        ...
        *,
        ...
        provenance = {
            series: {
                ...
                "expected_closed_boundary": row.get("expected_closed_boundary"),
