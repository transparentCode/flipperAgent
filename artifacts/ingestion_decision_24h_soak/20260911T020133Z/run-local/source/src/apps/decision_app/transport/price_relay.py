...
class PriceRelayPublisher:
    ...
    def __init__(
        ...
        *,
        ...
        if io_timeout_seconds is not None and (
        ...
        self._io_timeout_seconds = (
            None if io_timeout_seconds is None else float(io_timeout_seconds)
        )
    ...
    async def publish(
        ...
        try:
            returned_id = await run_with_timeout(
                self._client.xadd(
                    plan.stream_key,
    ...
    async def _exact_entry(
        ...
        raw = await run_with_timeout(
            self._client.xrange(stream_key, required_id, required_id),
            self._io_timeout_seconds,
            operation="price-relay exact-ID reconciliation",
        )
        if not raw:
            return None
    ...
    async def _stream_head(self, stream_key: str) -> str | None:
        raw = await run_with_timeout(
            self._client.xrevrange(stream_key, "+", "-", count=1),
            self._io_timeout_seconds,
            operation="price-relay stream-head reconciliation",
        )
        if not raw:
            return None
        if not isinstance(raw, Sequence):
...
class PriceRelay:
    ...
    async def bootstrap(self) -> Mapping[str, PriceRelayProgress]:
        ...
        for plan_id, plan in self._plans.items():
            ...
            try:
                raw = await run_with_timeout(
                    self._publisher._client.xrevrange(
                        plan.stream_key,
                        "+",
                        "-",
                        count=1,
                    ),
                    self._publisher._io_timeout_seconds,
                    operation="price-relay startup tail read",
                )
                if not raw:
