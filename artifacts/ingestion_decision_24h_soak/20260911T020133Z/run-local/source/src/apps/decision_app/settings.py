...
class DecisionDependencyIOSettings(BaseModel):
    """Explicit Decision-local budgets for dependency and control I/O."""
    ...
    control_wait_timeout_seconds: float = 30.0
    ...
    @classmethod
    def validate_seconds(cls, value: object, info: Any) -> float:
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise TypeError(f"{info.field_name} must be a finite numeric value")
        result = float(value)
    ...
    @model_validator(mode="after")
    def validate_relationships(self) -> DecisionDependencyIOSettings:
        if self.db_operation_timeout_seconds < self.io_timeout_seconds:
            raise ValueError(
                "dependency_io.db_operation_timeout_seconds must be at least "
                "io_timeout_seconds"
            )
        if self.control_wait_timeout_seconds < self.db_operation_timeout_seconds:
            raise ValueError(
                "dependency_io.control_wait_timeout_seconds must be at least "
                "db_operation_timeout_seconds"
            )
        return self
...
class DecisionGlobalSettings(BaseModel):
    ...
    dependency_io: DecisionDependencyIOSettings = Field(
        default_factory=DecisionDependencyIOSettings
    )
    ...
    @model_validator(mode="after")
    def validate_xread_budget(self) -> DecisionGlobalSettings:
        if self.live_input.block_ms / 1000 >= self.dependency_io.io_timeout_seconds:
            raise ValueError(
                "live_input.block_ms must be shorter than "
