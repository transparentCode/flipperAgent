"""Bounded, power-independent macOS sleep-assertion guard for a future
pre-soak wrapper.
...
Example (future wiring only)::
    ...
    .venv/bin/python scripts/host_sleep_guard.py \
        --max-check-gap 60 --probe-interval 5 -- \
        python -c 'print("harmless smoke")'

The guard has no power-source or remaining-battery gate.  A closed lid, forced
sleep, shutdown, or another physical interruption can still interrupt a run;
this guard reports an evidence gap and never treats an interrupted interval as
valid.
"""
...
REQUIRED_ASSERTION_TYPES = frozenset({"PreventUserIdleSystemSleep"})
...
@dataclass(frozen=True, slots=True)
class CleanupResult:
    """Bounded result for one owned process target."""

    success: bool
    leader_reaped: bool | None
    group_state: str
    failure_code: str | None = None

    @classmethod
    def not_started(cls) -> CleanupResult:
        return cls(
            success=True,
            leader_reaped=None,
...
class SleepAssertionGuard:
    ...
    def _terminate_group(self, process: Any) -> CleanupResult:
        ...
        def reap_leader(deadline: float) -> bool:
        ...
        def wait_group() -> tuple[bool | None, bool]:
            deadline = self._dependencies.monotonic() + self._config.terminate_grace
            leader_reaped = False
            while True:
                leader_reaped = reap_leader(deadline) or leader_reaped
                state = group_state()
                if state is not True:
                    return state, leader_reaped
                remaining = deadline - self._dependencies.monotonic()
                if remaining <= 0:
                    return True, leader_reaped
        ...
        def result_for_state(
            ...
            *,
            ...
            if state is False and leader_reaped and failure_code is None:
                return CleanupResult(
                    success=True,
                    leader_reaped=True,
                    group_state="ABSENT",
            ...
            if state is None:
                failure_code = "UNKNOWN_GROUP_STATE"
            elif state is True:
                failure_code = "STILL_ADDRESSABLE"
            else:
                failure_code = "LEADER_UNREAPED"
            return CleanupResult(
                success=False,
                leader_reaped=leader_reaped,
                group_state=(
        ...
        if state is None:
            deadline = self._dependencies.monotonic() + self._config.terminate_grace
            return result_for_state(state, reap_leader(deadline))
        if not state:
            deadline = self._dependencies.monotonic() + self._config.terminate_grace
            return result_for_state(state, reap_leader(deadline))
        try:
            self._dependencies.killpg(pid, signal.SIGTERM)
            state, leader_reaped = wait_group()
            if state is None:
                return result_for_state(state, leader_reaped)
            if not state:
                return result_for_state(state, leader_reaped)
            self._dependencies.killpg(pid, signal.SIGKILL)
            state, leader_reaped_after_kill = wait_group()
            return result_for_state(
                state,
                leader_reaped or leader_reaped_after_kill,
            )
        except ProcessLookupError:
            ...
            deadline = self._dependencies.monotonic() + self._config.terminate_grace
            leader_reaped = reap_leader(deadline)
            state = group_state()
            return result_for_state(state, leader_reaped)
        except OSError:
            deadline = self._dependencies.monotonic() + self._config.terminate_grace
            leader_reaped = reap_leader(deadline)
            return result_for_state(
                group_state(), leader_reaped, failure_code="SIGNAL_FAILED"
            )
    ...
    def run(self, command: Sequence[str]) -> int:
        ...
        with self._signal_handlers():
            try:
                ...
                for target, process in (("child", child), ("assertion", assertion)):
                    if process is None:
                        cleanup_results[target] = CleanupResult.not_started()
                        continue
                    try:
                        result = self._terminate_group(process)
                        cleanup_results[target] = result
                        if result.success is not True:
                            cleanup_failed = True
                    except Exception:  # noqa: BLE001
                        cleanup_results[target] = CleanupResult(
                            success=False,
                            leader_reaped=False,
                            group_state="UNKNOWN",
                            failure_code="CLEANUP_EXCEPTION",
                        )
                        cleanup_failed = True
                if child is not None and child_returncode is None:
                    try:
                        observed_returncode = child.poll()
                    except Exception:  # noqa: BLE001
                        observed_returncode = None
                    if observed_returncode is None:
                        prior_child_cleanup = cleanup_results.get("child")
                        if prior_child_cleanup is None or prior_child_cleanup.success:
                            cleanup_results["child"] = CleanupResult(
                                success=False,
                                leader_reaped=(
                                    prior_child_cleanup.leader_reaped
                                    if prior_child_cleanup is not None
                                    else None
                                ),
                                group_state=(
                                    prior_child_cleanup.group_state
                                    if prior_child_cleanup is not None
                                    else "UNKNOWN"
                                ),
                                failure_code="CHILD_STATUS_UNVERIFIED",
                            )
                        cleanup_failed = True
                    else:
                ...
                record = {
                    ...
                    "cleanup_results": {
                        target: result.as_record()
                        for target, result in cleanup_results.items()
                    },
                }
                self._emit(record)
        return exit_code
