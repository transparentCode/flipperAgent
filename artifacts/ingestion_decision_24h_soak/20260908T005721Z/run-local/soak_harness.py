...
class Soak(legacy.Soak):
    ...
    def sample(self, force: bool = False) -> None:
        if (
            ...
            return
        super().sample(force=force)
        now = time.monotonic()
        if (
            force
            or self.phase in {"warmup", "measurement"}
            and now - self.last_gate_at >= 15.0
        ):
            self.app_gate()
            self.last_gate_at = now
    ...
    def resource_sample(self) -> dict[str, Any]:
        ...
        core_records, measurement_records, all_run_records = self._split_records(
            records
        )
        core = self.stats(core_records)
        measurement = self.stats(measurement_records)
        run_stats = self.stats(all_run_records)
...
"""
    ...
    def app_gate(self) -> tuple[bool, dict[str, Any]]:
        probes = {
            "ingestion": legacy.http_get(PIPELINE_URLS["ingestion_runtime"], timeout=5),
            "decision": legacy.http_get(PIPELINE_URLS["decision_runtime"], timeout=5),
            "lanes": legacy.http_get(PIPELINE_URLS["decision_lanes"], timeout=5),
            "inputs": legacy.http_get(PIPELINE_URLS["decision_inputs"], timeout=5),
        }
        db = self.db_probe()
        records = self.service_containers()
        history = self.history_probe()
