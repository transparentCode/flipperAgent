"""Offline Phase 7H walk-forward runner for local follow-through candidates."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_md:
        ...
        Path(args.output_md).write_text(render_ft_walkforward_markdown(payload["matrix_report"]), encoding="utf-8")
    return 0
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    matrix = build_ft_walkforward_matrix_report(reports)
    return {
        "phase": "phase_7h_ft_walkforward_runner",
        "asset": args.asset,
        "timeframe": args.timeframe,
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 7H follow-through walk-forward validation.")
    ...
    parser.add_argument("--output-json", default="research/regime_v2_phase7h_ft_wf.json")
    parser.add_argument("--output-md", default="research/regime_v2_phase7h_ft_wf.md")
