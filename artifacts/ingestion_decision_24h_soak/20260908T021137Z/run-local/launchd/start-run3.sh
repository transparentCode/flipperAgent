...
RUN3_ID="20260908T021137Z"
RUN3_DIR="$RUN3_ROOT/artifacts/ingestion_decision_24h_soak/$RUN3_ID"
RUN3_PROJECT="flipper-id-soak-20260908t021137z"
...
if value.get("desired_state") != "running":
    raise SystemExit(1)
if value.get("state") not in {"starting", "live", "recovering"}:
    raise SystemExit(1)
...
for RUN3_RECOVERY_LANE in "BTC|BTC-USDT-PERP" "ETH|ETH-USDT-PERP"; do
  ...
  while true; do
    ...
    RUN3_RECOVERY_RESPONSE="$(curl -fsS --max-time "$RUN3_REMAINING" \
      ...
      2>/dev/null || true)"
    if print -r -- "$RUN3_RECOVERY_RESPONSE" | validate_recovery_response; then
      break
    fi
