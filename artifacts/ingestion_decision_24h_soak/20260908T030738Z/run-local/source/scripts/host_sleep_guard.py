...
class GuardFailure(RuntimeError):
    ...
    def __init__(self, reason_code: str, *, signal_number: int | None = None):
...
class HostProbe:
    ...
    def _run(
        self, argv: Sequence[str], *, timeout: Callable[[], float] | None = None
    ) -> str:
        try:
            ...
            del exc
            raise GuardFailure("PROBE_TIMEOUT") from None
        except (FileNotFoundError, OSError) as exc:
            del exc
            raise GuardFailure("MISSING_TOOL") from None
        if getattr(result, "returncode", None) != 0:
            raise GuardFailure("PROBE_FAILED")
        output = getattr(result, "stdout", None)
        if not isinstance(output, str):
            raise GuardFailure("PROBE_MALFORMED")
        return output
    ...
    def verify_environment(
        ...
        *,
        ...
        power_output = self._run((tools.pmset, "-g", "batt"), timeout=timeout)
        try:
            on_ac = parse_ac_power(power_output)
        except GuardFailure as exc:
            if runtime and exc.reason_code == "ON_BATTERY":
                raise GuardFailure("AC_LOST") from None
            raise
        if not on_ac:
            raise GuardFailure("ON_BATTERY")
        ...
        if not parse_lid_open(lid_output):
            raise GuardFailure("LID_CLOSED")
    ...
    def verify_assertion(
        ...
        *,
        ...
        output = self._run((tools.pmset, "-g", "assertions"), timeout=timeout)
    ...
    def _monitor(
        ...
        while True:
            ...
            if returncode is not None:
                ...
                self._verify_runtime(assertion, tools, last_check_at)
