"""SqueezeBreakout model — BB/KC squeeze detection with dual-KAMA crossover + TTM delta-linreg momentum."""
...
class SqueezeBreakoutModel(BaseModel):
    ...
    def evaluate(self, features: FeatureVector) -> ModelOutput:
        ...
        if len(self._close_buf) >= mom_period:
            ...
            if len(self._delta_buf) >= mom_period:
                # Compute linreg of delta buffer
        ...
        if direction != 0 and lr_mom is not None:
            if atr is not None and atr > 0:
                conviction = min(1.0, abs(lr_mom) / atr)
            else:
                conviction = 0.5
    ...
    def _batch_signal_strength_filter(
        ...
        """Apply v4 signal strength filter to batch directions."""
