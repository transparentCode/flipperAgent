"""DivergenceEdgeScorer optimization — Optuna objective with purged k-fold CV."""
...
import libs.models.divergence_edge.model  # noqa: F401

MODEL_NAME = "DivergenceEdgeScorer"
...
def make_objective(
    ...
    """Return an Optuna-compatible objective for DivergenceEdgeScorer.
...
def post_process_params(params: dict[str, Any]) -> dict[str, Any]:
    ...
    result = dict(params)
    for key in ("divergence_lookback", "min_confirming_indicators"):
        if key in result:
