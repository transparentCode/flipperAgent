...
class LaneEffectProgressRepository:
    ...
    def _ensure_usable(self) -> None:
        if self._poisoned:
            raise RuntimeError(
                "lane-effect repository is poisoned after lease cleanup failure"
            )
        ...
        async with acquire_db_connection(
            ...
            if callable(transaction):
                ...
                if deadline is not None:
                    require_remaining(
                        deadline,
                        operation="lane-effect transaction begin",
                    )
                await self._phase(tx.start(), deadline, "lane-effect transaction begin")
                try:
                    ...
                    try:
                        await cleanup_with_timeout(
                            ...
                            operation="lane-effect transaction rollback",
                            retained_tasks=self._retained_cleanup_tasks,
                        )
                ...
                if deadline is not None:
                    require_remaining(
                        deadline,
                        operation="lane-effect transaction commit",
                    )
                await self._phase(
                    tx.commit(),
                    deadline,
                    "lane-effect transaction commit",
                )
            else:
                result = await self._save_locked(
                    connection,
