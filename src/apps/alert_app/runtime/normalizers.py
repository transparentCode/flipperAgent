...
def normalize_ingestion_runtime_event(event: IngestionRuntimeEvent) -> NormalizedAlertEvent:
    mapped_type = {
    ...
    }.get(event.event_type, AlertEventType.INGESTION_RUNTIME_FAILURE)
    ...
    return NormalizedAlertEvent(
        ...
        recovery_key=(
            None
            if event.event_type == IngestionEventType.COMMAND_ACCEPTED
            else f"ingestion:{event.event_type.value}:{event.symbol}:{event.timeframe or 'na'}"
        ),
        emitted_at=event.emitted_at,
    )
