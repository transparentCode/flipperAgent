...
_SCHEMA_BOOTSTRAP_LOCK_ID = 48_216_421
...
def _is_idempotent_schema_error(exc: Exception) -> bool:
    ...
    duplicate_markers: Iterable[str] = (
        "already exists",
        "already a hypertable",
        "already a continuous aggregate",
    )
            ...
            for statement in statements:
                try:
                    ...
                    if _is_idempotent_schema_error(exc):
                        logger.info("Skipping existing ingestion schema statement: %s", statement.splitlines()[0])
                        continue
                    raise
        finally:
