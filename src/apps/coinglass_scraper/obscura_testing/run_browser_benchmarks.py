"""Benchmark CoinGlass extraction across Chromium and CDP-attached browsers."""
...
async def _run_benchmarks(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not args.skip_obscura:
        process: subprocess.Popen[str] | None = None
        try:
            if args.obscura_launch_command:
                process = subprocess.Popen(args.obscura_launch_command, shell=True)
                _wait_for_cdp(args.obscura_endpoint_url, args.startup_timeout_seconds)

            obscura_interceptor = ObscuraCDPHeatmapInterceptor(
                endpoint_url=args.obscura_endpoint_url,
    ...
    return {
        ...
        "exchange": args.exchange,
        "symbol": args.symbol,
        "obscura_endpoint_url": args.obscura_endpoint_url,
        "results": results,
    }
...
def main() -> None:
    ...
    parser.add_argument("--obscura-endpoint-url", default="http://127.0.0.1:9222")
    parser.add_argument("--obscura-launch-command", default=None)
    parser.add_argument("--skip-obscura", action="store_true")
    parser.add_argument("--print-default-obscura-command", action="store_true")
    parser.add_argument("--output-path", default=None)
    args = parser.parse_args()

    if args.print_default_obscura_command:
