"""CDP-attached CoinGlass scraper prototype for Lightpanda-based testing."""
...
def build_lightpanda_launch_command(
    binary_path: str = "lightpanda",
    host: str = "127.0.0.1",
    port: int = 9222,
) -> str:
    """Return a minimal Lightpanda launch command for local CDP testing."""
    return f"{binary_path} serve --host {host} --port {port}"


class LightpandaCDPHeatmapInterceptor(CoinGlassHeatmapInterceptor):
    """Fetch CoinGlass heatmaps by attaching Patchright to a Lightpanda CDP endpoint."""
    ...
    def __init__(
        ...
        self.endpoint_url = endpoint_url or self._config.get(
            "coinglass.lightpanda_testing.endpoint_url", "ws://127.0.0.1:9222"
        )
        self.connect_timeout_ms = int(
            self._config.get("coinglass.lightpanda_testing.connect_timeout_ms", 30000)
        )
        self.cdp_headers = cdp_headers or self._config.get(
            "coinglass.lightpanda_testing.cdp_headers"
        )
