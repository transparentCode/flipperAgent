...
class RawIndicatorPipeline:
    ...
    def process_tick(self, data: BarTuple) -> dict[str, Any]:
        ...
        for output_key, indicator in self._indicator_entries:
            if not indicator.is_primed:
                logger.warning("Indicator '%s' is not primed. Skipping update.", output_key)
                continue
    ...
    def snapshot_features(self, historical_data: Sequence[BarTuple]) -> dict[str, Any]:
        ...
        for output_key, indicator in self._indicator_entries:
            ...
            try:
                ...
                if not mapped_data:
                    continue
                batch_input = pd.DataFrame(mapped_data) if isinstance(mapped_data[0], dict) else mapped_data
    ...
    def _initialize_indicators(self) -> None:
        try:
            ...
            logger.error("Error reading indicator config", exc_info=True)
            return

        for config_key, params in timeframe_node.items():
            try:
                indicator_type, output_key, constructor_params = _resolve_indicator_config(config_key, params)
                ...
                logger.warning("Indicator type for '%s' not found in registry. Skipping.", config_key)
            except Exception:
                logger.error("Error instantiating '%s'", config_key, exc_info=True)
...
def _resolve_indicator_config(config_key: str, params: Any) -> tuple[str, str, dict[str, Any]]:
    if isinstance(params, dict) and "type" in params:
        indicator_type = str(params["type"])
        output_key = config_key
        constructor_params = {key: value for key, value in params.items() if key != "type"}
