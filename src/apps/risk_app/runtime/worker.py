...
from libs.risk.profile_resolver import resolve_risk_config
...
class RiskWorker(BaseStreamConsumer):
    ...
    async def _process_signal_batch(self, signals: list[TradeSignal]) -> None:
        ...
        batch_risk_config = self._resolve_batch_risk_config(signals)
        ...
        for signal in to_assess:
            effective_risk_config = resolve_risk_config(
                self.risk_config,
                asset=signal.asset,
    ...
    def _resolve_batch_risk_config(self, signals: list[TradeSignal]) -> dict[str, Any]:
        model_names = {
            str(signal.model_name).strip()
            for signal in signals
            if str(signal.model_name).strip()
        }
        if len(model_names) != 1:
            return self.risk_config
        return resolve_risk_config(
            self.risk_config,
            asset=self.asset,
