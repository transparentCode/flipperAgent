"""Offline Phase 7P setup-origin transition candidate runner."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_md:
        ...
        Path(args.output_md).write_text(render_setup_transition_markdown(payload["matrix_report"]), encoding="utf-8")
    return 0
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    matrix = build_setup_transition_matrix_report(reports)
    return {
        "phase": "phase_7p_setup_transition_runner",
        "asset": args.asset,
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 7P setup-origin transition candidates.")
    ...
    parser.add_argument("--output-json", default="research/regime_v2_phase7p_transition_setup.json")
    parser.add_argument("--output-md", default="research/regime_v2_phase7p_transition_setup.md")
    args = parser.parse_args(argv)
    args.asset = args.asset or ["BNBUSDT", "ETHUSDT", "BTCUSDT"]
    args.timeframe = args.timeframe or ["1h"]
    args.lookback_bars = args.lookback_bars or [8, 12, 20]
    args.min_candidate_score = args.min_candidate_score or [0.58, 0.62, 0.66]
