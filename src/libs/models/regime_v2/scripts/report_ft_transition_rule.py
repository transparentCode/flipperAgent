"""Offline Phase 7L split-local transition-rule retest runner."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_md:
        ...
        Path(args.output_md).write_text(render_ft_transition_rule_markdown(payload["matrix_report"]), encoding="utf-8")
    return 0
    ...
    for threshold in args.threshold:
        reports.append(
            build_ft_transition_rule_retest_report(
                ...
                allowed_market_phases=tuple(args.allowed_market_phase),
                allowed_horizon_biases=tuple(args.allowed_horizon_bias),
                action=args.action,
            )
        )
    matrix = build_ft_transition_rule_matrix_report(reports)
    return {
        "phase": "phase_7l_ft_transition_rule_runner",
        "asset": args.asset,
        "timeframe": args.timeframe,
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 7L split-local follow-through transition rule.")
    ...
    parser.add_argument("--allowed-market-phase", action="append", default=None)
    parser.add_argument("--allowed-horizon-bias", action="append", default=None)
    parser.add_argument("--action", choices=("reverse_direction", "suppress"), default="reverse_direction")
    parser.add_argument("--output-json", default="research/regime_v2_phase7l_ft_transition_rule.json")
    parser.add_argument("--output-md", default="research/regime_v2_phase7l_ft_transition_rule.md")
    ...
    args.target_split = args.target_split or [2]
    args.transition_direction = args.transition_direction or ["down"]
    args.allowed_market_phase = args.allowed_market_phase or ["compressed_wait", "breakout_setup"]
    args.allowed_horizon_bias = args.allowed_horizon_bias or ["wait_for_expansion", "mid", "short_to_mid"]
    return args
