"""CLI for PA paper pause-rule search."""
...
from libs.selection.regime_v2_pa_paper_gate_search import (
    build_pa_paper_gate_search_report,
    render_pa_paper_gate_search_markdown,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_md:
        ...
        Path(args.output_md).write_text(render_pa_paper_gate_search_markdown(report), encoding="utf-8")
    return 0
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = build_pa_paper_gate_search_report(
        records,
        ohlcv_by_pair,
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Search PA paper pause-rule variants.")
    ...
    parser.add_argument("--output-json", default="research/regime_v2_pa_paper_gs.json")
    parser.add_argument("--output-md", default="research/regime_v2_pa_paper_gs.md")
