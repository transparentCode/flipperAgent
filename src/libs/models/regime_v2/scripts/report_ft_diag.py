"""Offline Phase 7I diagnostics runner for failed follow-through windows."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_md:
        ...
        Path(args.output_md).write_text(render_ft_failure_diagnostics_markdown(payload["matrix_report"]), encoding="utf-8")
    return 0
    ...
    for threshold in args.threshold:
        ...
        reports.append(
            build_ft_failure_diagnostics(
                ...
                failed_split_indices=tuple(args.failed_split),
                horizons=tuple(args.horizon),
                fees_bps=tuple(args.fee_bps),
            )
        )
    matrix = build_ft_failure_diagnostics_matrix(reports)
    return {
        "phase": "phase_7i_ft_diagnostics_runner",
        "asset": args.asset,
        "timeframe": args.timeframe,
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 7I follow-through failure diagnostics.")
    ...
    parser.add_argument("--failed-split", action="append", type=int, default=None)
    ...
    parser.add_argument("--output-json", default="research/regime_v2_phase7i_ft_diag.json")
    parser.add_argument("--output-md", default="research/regime_v2_phase7i_ft_diag.md")
    args = parser.parse_args(argv)
    args.threshold = args.threshold or [0.25, 0.30]
    args.failed_split = args.failed_split or [2, 3]
