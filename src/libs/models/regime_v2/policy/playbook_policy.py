...
def evidence_to_policy(evidence: RegimeEvidence, config: PolicyConfig) -> RegimePolicy:
    ...
    if evidence.shock_risk >= 0.85:
        reasons.append("shock_risk_extreme")
    if evidence.liquidity_stress >= 0.85:
    ...
    max_position_scale = _position_scale(evidence, any([allow_trend, allow_breakout, allow_mr, allow_scalping, allow_countertrend]))
    stop_multiplier = _stop_multiplier(evidence)
    target_multiplier = _target_multiplier(evidence, allow_trend=allow_trend, allow_breakout=allow_breakout)
...
def _playbook_scores(evidence: RegimeEvidence, config: PolicyConfig) -> dict[str, float]:
    ...
    uncertainty_soft_penalty = 1.0 - 0.35 * evidence.uncertainty
    ...
    trend_score = (
        ...
        * _soft_threshold(evidence.trend_strength, config.trend_min_strength)
        * _inverse_soft_threshold(evidence.chop_risk, config.trend_max_chop)
        * (0.70 + 0.30 * evidence.trend_persistence)
    )

    breakout_setup_score = (
        confidence_gate
        * _soft_threshold(evidence.pre_breakout_setup_score, 0.58)
        * _inverse_soft_threshold(evidence.structural_break_risk, 0.45)
        * _inverse_soft_threshold(evidence.shock_risk, 0.55)
    )
    displacement_breakout_score = (
        confidence_gate
        * _soft_threshold(evidence.displacement_breakout_score, config.breakout_min_quality)
        * _inverse_soft_threshold(evidence.false_breakout_risk, config.breakout_max_false_break)
        * _inverse_soft_threshold(evidence.shock_risk, 0.80)
    )
    retest_breakout_score = (
        confidence_gate
        * _soft_threshold(evidence.post_breakout_retest_score, 0.35)
        * _inverse_soft_threshold(evidence.false_breakout_risk, config.breakout_max_false_break)
        * _inverse_soft_threshold(evidence.structural_break_risk, 0.65)
    )
    breakout_score = max(displacement_breakout_score, retest_breakout_score)

    mr_context = max(evidence.range_quality, evidence.compression_score * 0.75)
    mean_reversion_score = (
        confidence_gate
        * _soft_threshold(evidence.mean_reversion_score, config.mr_min_score)
        * _soft_threshold(mr_context, 0.45)
        * _inverse_soft_threshold(evidence.structural_break_risk, config.mr_max_break_risk)
    )

    scalping_score = (
        confidence_gate
        * _inverse_soft_threshold(evidence.shock_risk, config.scalping_max_shock)
        * _inverse_soft_threshold(evidence.liquidity_stress, 0.65)
        * (0.55 + 0.45 * max(evidence.trend_strength, evidence.range_quality, evidence.breakout_quality))
    )

    countertrend_score = (
        confidence_gate
        * _soft_threshold(evidence.range_quality, config.countertrend_min_range)
        * _inverse_soft_threshold(evidence.trend_strength, 0.45)
        * _inverse_soft_threshold(evidence.structural_break_risk, 0.45)
    )
...
def _playbook_score_floor(config: PolicyConfig) -> float:
    ...
    """
    return max(0.20, min(0.40, config.min_confidence * 0.80))
...
def _position_scale(evidence: RegimeEvidence, any_allowed: bool) -> float:
    if not any_allowed:
        return 0.0
    risk_penalty = max(evidence.uncertainty, evidence.shock_risk, evidence.liquidity_stress * 0.8)
    scale = evidence.confidence * (1.0 - 0.65 * risk_penalty)
    if evidence.structural_break_risk > 0.70 and evidence.breakout_quality < 0.65:
        scale *= 0.50
    return round(float(clip01(scale)), 4)


def _stop_multiplier(evidence: RegimeEvidence) -> float:
    base = 1.0 + 0.75 * evidence.shock_risk + 0.35 * evidence.structural_break_risk
    if evidence.volatility_state in {"expanding", "shock"}:
        base += 0.25
    return round(float(max(0.7, min(base, 2.5))), 4)


def _target_multiplier(evidence: RegimeEvidence, *, allow_trend: bool, allow_breakout: bool) -> float:
    base = 1.0
    if allow_trend:
        base += 0.45 * evidence.trend_strength
    if allow_breakout:
        base += 0.35 * evidence.breakout_quality
    if evidence.chop_risk > 0.70:
        base *= 0.75
    return round(float(max(0.6, min(base, 2.2))), 4)


def _holding_period(evidence: RegimeEvidence, config: PolicyConfig) -> int:
    base = config.base_holding_period
    if evidence.trend_strength > 0.65 and evidence.chop_risk < 0.45:
        base = int(base * 1.5)
    if evidence.mean_reversion_score > 0.65 or evidence.shock_risk > 0.65:
        base = int(base * 0.6)
    return max(1, base)
