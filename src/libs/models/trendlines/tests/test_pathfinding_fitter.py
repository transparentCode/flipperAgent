...
def test_pathfinding_ols_on_path_refits_using_all_selected_points():
    path = [(0, 0.0), (5, 8.0), (10, 10.0)]
    endpoint = PathfindingFitter(pivot_window=1, line_fit_mode="endpoint")._build_line(
        path=path,
        is_support=True,
        total_bars=11,
    )
    ols = PathfindingFitter(pivot_window=1, line_fit_mode="ols_on_path")._build_line(
        path=path,
        is_support=True,
        total_bars=11,
    )
    ...
    assert endpoint.start_index == 5
    assert endpoint.end_index == 10
    assert endpoint.slope == pytest.approx(0.4)
    assert ols.start_index == 0
    assert ols.end_index == 10
    assert ols.slope == pytest.approx(1.0)
    assert ols.intercept == pytest.approx(1.0)
    assert ols.metadata["line_fit_mode"] == "ols_on_path"


def test_pathfinding_rejects_unknown_line_fit_mode():
    fitter = PathfindingFitter(pivot_window=1, line_fit_mode="bad_mode")

    with pytest.raises(ValueError, match="line_fit_mode"):
        fitter.fit(_make_pathfinding_frame())
