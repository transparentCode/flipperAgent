"""RegimeRelativeValueScorer optimization — Optuna objective with purged k-fold CV."""
...
import libs.models.regime_relative_value.model  # noqa: F401

MODEL_NAME = "RegimeRelativeValueScorer"
...
# regime_state_required is the model's core hypothesis (RISK_OFF = 0) — always fixed.
...
def make_objective(
    ...
    """Return an Optuna-compatible objective for RegimeRelativeValueScorer.
...
def post_process_params(params: dict[str, Any]) -> dict[str, Any]:
    ...
    result = dict(params)
    for key in ("rsi_oversold_gate", "regime_state_required"):
        if key in result:
