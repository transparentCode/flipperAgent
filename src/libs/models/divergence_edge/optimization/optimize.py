"""CLI runner for DivergenceEdgeScorer optimization.
...
from libs.models.divergence_edge.optimization import optimizer as de_optimizer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run DivergenceEdgeScorer optimization study"
    )
...
def main() -> None:
    ...
    # --- Build objective ---
    objective_fn = de_optimizer.make_objective(
        feature_df, timeframe=args.timeframe, cost_bps=args.cost_bps,
    )

    # --- Study config ---
    n_trials = args.n_trials or de_optimizer.STUDY_DEFAULTS.get("n_trials", 300)
    study_config = StudyConfig(
        model_name=de_optimizer.MODEL_NAME,
        asset=args.asset,
        timeframe=args.timeframe,
        n_trials=n_trials,
        sampler=de_optimizer.STUDY_DEFAULTS.get("sampler", "TPE"),
        pruner=de_optimizer.STUDY_DEFAULTS.get("pruner", "MedianPruner"),
        objectives=["score"],
        directions=[de_optimizer.STUDY_DEFAULTS.get("direction", "maximize")],
    )
    ...
    if args.write_back:
        write_best_params(
            de_optimizer.MODEL_NAME, args.asset, args.timeframe, processed_params,
        )
