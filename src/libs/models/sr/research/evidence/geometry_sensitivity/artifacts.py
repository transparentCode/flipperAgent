...
def _regular_members(path: Path, *, expected: set[str]) -> bool:
    """Return whether a bundle contains exactly the required regular members."""
    try:
    ...
    return (
        ...
        and all(item.is_file() and not item.is_symlink() for item in members)
    )
...
def _atomic_publish(path: Path, files: dict[str, bytes]) -> None:
    ...
    if path.exists():
        if not _regular_members(path, expected=set(files)):
...
def _validate_manifest(path: Path) -> dict[str, Any]:
    ...
    if not _regular_members(path, expected=expected_members):
