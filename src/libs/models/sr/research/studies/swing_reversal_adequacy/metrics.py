"""Paired real-wick minus same-side naïve-band V2.2 metrics."""
...
def build_study(
    ...
    *,
    ...
    if (
        ...
        or any(type(item) is not SwingRecord for item in swings)
        ...
        raise ContractValidationError("V2.2 metrics require typed cases and controls")
    if len({case.case_id for case in cases}) != len(cases):
        raise ContractValidationError("V2.2 metrics require unique cases")
    ...
    if observed != expected:
        raise ContractValidationError(
            "V2.2 controls must contain exact ordered SUPPORT/RESISTANCE topology per in-fold case"
        )
