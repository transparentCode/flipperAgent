"""Causal swing-reversal cases and matched naïve-band outcomes."""
...
from .contracts import CandidateCase, NaiveControl, OutcomeStatus, SwingRecord
...
def build_model_bars(
    capsule: SourceCapsule, *, config: SwingReversalAdequacyConfig
) -> tuple[ClosedBar, ...]:
    if (
        type(capsule) is not SourceCapsule
        or type(config) is not SwingReversalAdequacyConfig
    ):
        raise ContractValidationError("V2.2 model requires typed capsule/configuration")
...
def _fold_for(timestamp: datetime, config: SwingReversalAdequacyConfig) -> str | None:
    return next(
        (fold.name for fold in config.folds if fold.start <= timestamp < fold.end), None
    )


def _fold_end(name: str, config: SwingReversalAdequacyConfig) -> datetime:
    for fold in config.folds:
        if fold.name == name:
            return fold.end
    raise ContractValidationError("band references unknown V2.2 fold")
...
def evaluate_candidates(
    bars: tuple[ClosedBar, ...], *, config: SwingReversalAdequacyConfig
) -> tuple[tuple[SwingRecord, ...], tuple[CandidateCase, ...]]:
    if type(bars) is not tuple or any(type(bar) is not ClosedBar for bar in bars):
        raise ContractValidationError("V2.2 candidates require ClosedBar tuple")
    swings: list[SwingRecord] = []
    cases: list[CandidateCase] = []
    for swing in detect_causal_swing_reversals(bars, config.detector):
        extreme, confirmation = (
            bars[swing.extreme_index],
            bars[swing.confirmation_index],
        )
        candidate = swing.candidate
        swings.append(
            SwingRecord(
                side=swing.side,
                extreme_bar_id=extreme.bar_id,
                extreme_index=swing.extreme_index,
                confirmation_bar_id=confirmation.bar_id,
                confirmation_index=swing.confirmation_index,
                extreme_atr=swing.extreme_atr,
        ...
        if candidate is None:
            continue
        if (
            candidate.formed_at != extreme.closed_at
            or candidate.available_at != confirmation.closed_at
            or candidate.atr_at_creation != confirmation.atr_at_close
            or swing.extreme_atr != extreme.atr_at_close
        ):
            raise ContractValidationError(
                "swing candidate provenance does not reconcile"
            )
        width_atr = (
        ...
        if swing.confirmation_index <= 0:
            raise ContractValidationError(
                "swing confirmation must have a predecessor bar"
            )
        cases.append(
            CandidateCase(
                candidate=candidate,
                ...
                extreme_index=swing.extreme_index,
                extreme_atr=swing.extreme_atr,
                prior_close=bars[swing.confirmation_index - 1].close,
                fold=fold,
                status=status,
                outcome=outcome,
    ...
    if len({case.candidate.candidate_id for case in cases}) != len(cases):
        raise ContractValidationError("V2.2 candidates must be unique")
    return tuple(swings), tuple(cases)
...
def build_naive_controls(
    ...
    *,
    ...
    if (
        ...
        raise ContractValidationError("V2.2 controls require typed cases/bars")
    ...
    for case in cases:
        ...
        if (
            ...
            or bars[case.extreme_index].bar_id != case.extreme_bar_id
            or bars[case.extreme_index].atr_at_close != case.extreme_atr
        ):
            raise ContractValidationError("control swing provenance cannot be aligned")
