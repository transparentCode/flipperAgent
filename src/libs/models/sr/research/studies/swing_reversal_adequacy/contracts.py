"""Immutable causal records for the SR-V2.2 swing-reversal study."""
...
_NAIVE_SOURCE = "prior_close_naive_v2_2"
_GATE_TOPOLOGY = (
    (
...
class SwingReversalDisposition(str, Enum):
    BEATS_NAIVE_NULL = "SWING_REVERSAL_BEATS_NAIVE_NULL"
    NOT_BETTER_THAN_NAIVE_NULL = "SWING_REVERSAL_NOT_BETTER_THAN_NAIVE_NULL"
    INSUFFICIENT_EVIDENCE = "INSUFFICIENT_EVIDENCE"


@dataclass(frozen=True)
class SwingRecord:
    """Causal state-machine confirmation, including zero-wick transitions."""
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("extreme_bar_id", "confirmation_bar_id"):
            object.__setattr__(
                self, name, _string(getattr(self, name), path=f"swing.{name}")
            )
        for name in ("extreme_index", "confirmation_index"):
            object.__setattr__(
                self, name, _integer(getattr(self, name), path=f"swing.{name}")
            )
        ...
        object.__setattr__(
            self,
            "extreme_atr",
            _finite(self.extreme_atr, path="swing.extreme_atr", minimum=0.0),
        )
        ...
        if self.candidate_id is not None:
            object.__setattr__(
                self,
                "candidate_id",
                _string(self.candidate_id, path="swing.candidate_id"),
            )

    def to_payload(self) -> dict[str, object]:
        return {
            name: getattr(self, name) if name != "side" else self.side.value
...
class CandidateCase:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "extreme_bar_id",
            _string(self.extreme_bar_id, path="case.extreme_bar_id"),
        )
        object.__setattr__(
            self,
            "extreme_index",
            _integer(self.extreme_index, path="case.extreme_index"),
        )
        if self.extreme_index >= self.confirmation_index:
        ...
        object.__setattr__(
            self,
            "extreme_atr",
            _finite(self.extreme_atr, path="case.extreme_atr", minimum=0.0),
        )
        if self.extreme_atr <= 0.0:
            raise ContractValidationError("case.extreme_atr must be positive")
        object.__setattr__(
            self,
            "prior_close",
...
class GateResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.threshold != expected[1]:
            raise ContractValidationError(
                "gate threshold is not the approved V2.2 value"
            )
...
@dataclass(frozen=True)
class Decision:
    disposition: SwingReversalDisposition
    gates: tuple[GateResult, ...]
    reason: str

    def __post_init__(self) -> None:
        if (
            type(self.disposition) is not SwingReversalDisposition
        ...
        if any(not item.passed for item in readiness):
            expected_disposition = SwingReversalDisposition.INSUFFICIENT_EVIDENCE
            expected_reason = "readiness gates failed"
        elif all(item.passed for item in utility):
            expected_disposition = SwingReversalDisposition.BEATS_NAIVE_NULL
            expected_reason = "all utility gates passed after readiness"
        else:
            expected_disposition = SwingReversalDisposition.NOT_BETTER_THAN_NAIVE_NULL
...
@dataclass(frozen=True)
class SwingReversalStudy:
    implementation_commit: str
    config_hash: str
    source_bundle_id: str
    source_capsule_bundle_id: str
    source_id: str
    swings: tuple[SwingRecord, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.swings) is not tuple
            or any(type(item) is not SwingRecord for item in self.swings)
        ...
        if any(
            left.side is right.side for left, right in zip(self.swings, self.swings[1:])
        ):
            raise ContractValidationError(
                "confirmed swing records must alternate sides"
            )
        cases_by_candidate = {item.candidate.candidate_id: item for item in self.cases}
        emitted = {
            item.candidate_id for item in self.swings if item.candidate_id is not None
        }
        if emitted != set(cases_by_candidate):
            raise ContractValidationError(
                "swing/case emitted candidate topology does not reconcile"
            )
        for swing in self.swings:
            if swing.candidate_id is None:
                continue
            case = cases_by_candidate[swing.candidate_id]
            if (
                case.candidate.side is not swing.side
                or case.extreme_bar_id != swing.extreme_bar_id
                or case.extreme_index != swing.extreme_index
                or case.confirmation_bar_id != swing.confirmation_bar_id
                or case.confirmation_index != swing.confirmation_index
                or case.extreme_atr != swing.extreme_atr
            ):
                raise ContractValidationError(
                    "swing/case causal provenance does not reconcile"
                )
        for name in (
