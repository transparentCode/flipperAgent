"""Network-free orchestration for frozen SR-V2.1 evidence."""
...
def repository_commit(repo_root: str | Path) -> str:
    try:
        ...
        raise ContractValidationError(
            "cannot determine V2.1 implementation commit"
        ) from exc


def load_frozen_inputs(
    config: PivotRejectionAdequacyConfig, *, repo_root: str | Path
) -> FrozenInputs:
    if type(config) is not PivotRejectionAdequacyConfig:
        raise ContractValidationError("V2.1 requires typed configuration")
    ...
    if (
        ...
        raise ContractValidationError(
            "frozen TAOUSDT source identity does not match V2.1 configuration"
        )
    ...
    if capsule.source_bundle_id != config.source.source_bundle_id:
        raise ContractValidationError(
            "canonical source capsule identity does not match V2.1 source"
        )
    return FrozenInputs(capsule, build_model_bars(capsule, config=config))


def compute_pivot_rejection_study(
...
def run_study(
    ...
    *,
    ...
    config = load_pivot_rejection_adequacy_config(str(config_path))
