"""Strict, fully frozen configuration for the SR-V2.1 study."""
...
from libs.models.sr.detection.pivot_rejection import PivotRejectionConfig
...
CONFIG_VERSION = "1"
TRIAL_NAME = "sr-v2.1-taousdt-1d-pivot-rejection-adequacy"
...
APPROVED_DETECTOR = {"pivot_span_bars": 5}
APPROVED_GATES = {
...
STAGE = "pivot_rejection_adequacy_development"
...
class FrozenSection:
    ...
    def __post_init__(self) -> None:
        if type(self.payload) is not dict or self.payload != self.expected:
            raise ContractValidationError(
                f"{self.name} is not the approved immutable V2.1 payload"
            )
...
class AdequacyGates:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ContractValidationError(
                "adequacy gates are not the approved immutable V2.1 payload"
            )
...
class ArtifactProtocol:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ContractValidationError(
                "artifact protocol must use the exact V2.1 stage and members"
            )
...
@dataclass(frozen=True)
class PivotRejectionAdequacyConfig:
    version: str
    trial_name: str
    venue: str
    asset: str
    ...
    def __post_init__(self) -> None:
        if _string(self.version, path="version") != CONFIG_VERSION or (
            ...
            raise ContractValidationError(
                "trial scope is outside the approved V2.1 TAOUSDT/1d study"
            )
        if (
            type(self.source) is not FrozenSource
            or type(self.atr) is not FrozenSection
            or type(self.detector) is not PivotRejectionConfig
            ...
            raise ContractValidationError(
                "V2.1 configuration sections have invalid types"
            )
        if (
            self.atr.name != "atr"
            ...
            raise ContractValidationError("V2.1 configuration protocol is not approved")
        if (
            ...
            raise ContractValidationError(
                "V2.1 folds, controls, or dispositions are not frozen"
            )
...
def load_pivot_rejection_adequacy_config(path: str) -> PivotRejectionAdequacyConfig:
    raw = _mapping(
        load_strict_research_yaml(
            path, description="V2.1 pivot-rejection configuration"
        ),
        path="config",
    )
    require_exact_keys(
    ...
    if (
        ...
        raise ContractValidationError("V2.1 list protocol is invalid")
    folds = tuple(
        CohortFold(
    ...
    return PivotRejectionAdequacyConfig(
        ...
        detector=PivotRejectionConfig(
            require_integer(
                detector["pivot_span_bars"], path="detector.pivot_span_bars", minimum=1
            )
        ),
