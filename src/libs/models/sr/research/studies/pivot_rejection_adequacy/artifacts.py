"""Immutable publication and semantic validation for SR-V2.1 evidence."""
...
from .contracts import PivotRejectionStudy
...
def _semantic(
    ...
    *,
    ...
    if study.config_hash != config.config_hash:
        raise ContractValidationError("V2.1 study/config identity mismatch")
    return {
...
def publish_study_bundle(
    ...
    *,
    ...
    if (
        type(study) is not PivotRejectionStudy
        or type(config) is not PivotRejectionAdequacyConfig
    ):
        raise ContractValidationError("V2.1 publication requires typed study/config")
    ...
    publish_immutable_directory(
        ...
        description="V2.1 pivot-rejection evidence",
    )
    return bundle_id, destination


def _validate_manifest(
    path: Path, config: PivotRejectionAdequacyConfig
) -> dict[str, Any]:
    if (
        not path.is_dir()
        or path.is_symlink()
        ...
        raise ContractValidationError("V2.1 artifact member set mismatch")
    members = tuple(path / name for name in config.artifact.members)
    for member in members:
        require_regular_file(member, description="V2.1 artifact member")
    manifest = load_strict_json(
        path / "manifest.json",
        description="V2.1 manifest",
        value_description="V2.1 artifact",
    ...
    if (
        ...
        raise ContractValidationError("V2.1 manifest identity/schema mismatch")
    if (
        ...
        raise ContractValidationError("V2.1 manifest config/semantic binding mismatch")
    raw_members = semantic["members"]
    if type(raw_members) is not list or len(raw_members) != 2:
        raise ContractValidationError("V2.1 manifest member metadata is invalid")
    ...
    for item in raw_members:
        ...
        metadata[item["name"]] = validate_member_metadata(
            item, expected_name=item["name"], description="V2.1 evidence member"
        )
    if set(metadata) != {"study.json", "cases.json"}:
        raise ContractValidationError("V2.1 manifest member names are invalid")
    for name, item in metadata.items():
        if member_metadata(name, (path / name).read_bytes()) != item:
            raise ContractValidationError(f"V2.1 artifact member hash mismatch: {name}")
    return manifest
...
def validate_study_bundle(
    ...
    *,
    ...
    if type(config) is not PivotRejectionAdequacyConfig:
        raise ContractValidationError("V2.1 validation requires typed configuration")
    reject_symlink_components(Path(path), description="V2.1 artifact")
    ...
    if expected_bundle_id is not None and manifest["bundle_id"] != expected_bundle_id:
        raise ContractValidationError(
            "V2.1 bundle ID does not match requested identity"
        )
    commit = (
    ...
    if semantic["implementation_commit"] != commit:
        raise ContractValidationError("V2.1 implementation identity mismatch")
    from .runner import compute_pivot_rejection_study

    recomputed = compute_pivot_rejection_study(
        config, repo_root=repo_root, implementation_commit=commit
    )
    study_payload = load_strict_json(
        bundle_path / "study.json",
        description="V2.1 study",
        value_description="V2.1 artifact",
        require_regular=False,
    )
    cases_payload = load_strict_json(
        bundle_path / "cases.json",
        description="V2.1 cases",
        value_description="V2.1 artifact",
        require_regular=False,
    )
    if (
        ...
        raise ContractValidationError(
            "V2.1 evidence does not match semantic recomputation"
        )
    expected = _semantic(
        recomputed,
        config,
    ...
    if semantic != expected or manifest["bundle_id"] != deterministic_hash(expected):
        raise ContractValidationError(
            "V2.1 manifest does not match semantic recomputation"
        )
    return recomputed
