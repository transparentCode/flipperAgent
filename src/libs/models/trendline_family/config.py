...
class ResolvedTrendlineFamilyConfig:
    ...
    @classmethod
    def create(cls, *, asset: str, timeframe: str, config_version: str, config: TrendlineFamilyConfig, field_provenance: Mapping[str, str]) -> "ResolvedTrendlineFamilyConfig":
        tracking_payload = {
            "asset": asset,
            "timeframe": timeframe,
        ...
        return cls(asset=asset, timeframe=timeframe, config_version=config_version, model=config.model, candidate=config.candidate,
                   ...
                   mtf_config_hash=deterministic_hash(mtf_payload))
