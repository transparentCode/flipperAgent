"""CLI runner for RegimePullbackScorer optimization.
...
from libs.models.regime_pullback.optimization import optimizer as rp_optimizer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RegimePullbackScorer optimization study"
    )
...
def main() -> None:
    ...
    # --- Build objective ---
    objective_fn = rp_optimizer.make_objective(
        feature_df, timeframe=args.timeframe, cost_bps=args.cost_bps,
    )

    # --- Study config ---
    n_trials = args.n_trials or rp_optimizer.STUDY_DEFAULTS.get("n_trials", 200)
    study_config = StudyConfig(
        model_name=rp_optimizer.MODEL_NAME,
        asset=args.asset,
        timeframe=args.timeframe,
        n_trials=n_trials,
        sampler=rp_optimizer.STUDY_DEFAULTS.get("sampler", "TPE"),
        pruner=rp_optimizer.STUDY_DEFAULTS.get("pruner", "MedianPruner"),
        objectives=["score"],
        directions=[rp_optimizer.STUDY_DEFAULTS.get("direction", "maximize")],
    )
    ...
    if args.write_back:
        write_best_params(
            rp_optimizer.MODEL_NAME, args.asset, args.timeframe, processed_params,
        )
