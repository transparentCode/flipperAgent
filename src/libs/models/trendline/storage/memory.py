"""Deterministic in-memory current-head and historical trendline repository."""
...
class InMemoryTrendlineRepository:
    ...
    def __init__(self) -> None:
        ...
        self._snapshot_history: dict[tuple[str, str], list[TrendlineFamilySnapshot]] = {}
        self._families: dict[str, TrendlineFamilyState] = {}
        self._family_history: dict[str, list[TrendlineFamilyState]] = {}
        self._events: dict[tuple[str, str, str], list[FamilyInteractionEvent]] = {}
    ...
    def save_snapshot(self, snapshot: TrendlineFamilySnapshot) -> None:
        ...
        self._validate_snapshot_families(snapshot)
        self._validate_snapshot_events(snapshot)
        stored = deserialize_snapshot(serialize_snapshot(snapshot))
        self._snapshots[key] = stored
        self._snapshot_history.setdefault(key, []).append(stored)
        for family in stored.active_families + stored.dormant_families:
            self._record_family(family)
        for event in stored.interaction_events:
            self._record_event(event)

    def save_family(self, family: TrendlineFamily) -> None:
        if not isinstance(family, TrendlineFamilyState):
            raise ContractValidationError("family must use TrendlineFamilyState")
        self._record_family(family)

    def save_event(self, event: TrendlineEvent) -> None:
        if not isinstance(event, FamilyInteractionEvent):
            raise ContractValidationError("event must use FamilyInteractionEvent")
        self._record_event(event)

    def get_family(self, family_id: str) -> TrendlineFamily | None:
        if not isinstance(family_id, str) or not family_id:
            raise ContractValidationError("family_id must be a non-empty string")
        family = self._families.get(family_id)
        return None if family is None else self._clone_family(family)
    ...
    def get_state_at(self, *, asset: str, timeframe: str, as_of: datetime) -> TrendlineContext:
        ...
        known_at = require_utc(as_of, field_name="as_of")
        key = (asset, timeframe)
        history = self._snapshot_history.get(key, ())
        snapshot = next((item for item in reversed(history) if item.timestamp <= known_at), None)
        families = () if snapshot is None else snapshot.active_families + snapshot.dormant_families
        events = self._events_at(asset=asset, timeframe=timeframe, as_of=known_at)
        return TrendlineContext(
            asset=asset,
            timeframe=timeframe,
            as_of=known_at,
            families=tuple(sorted((self._clone_family(family) for family in families), key=lambda family: family.family_id)),
            events=events,
        )

    def _validate_snapshot_events(self, snapshot: TrendlineFamilySnapshot) -> None:
        for event in snapshot.interaction_events:
            self._validate_event_record(event)

    def _validate_snapshot_families(self, snapshot: TrendlineFamilySnapshot) -> None:
        for family in snapshot.active_families + snapshot.dormant_families:
            self._validate_family_record(family)

    def _validate_family_record(self, family: TrendlineFamilyState) -> None:
        previous = self._families.get(family.family_id)
        if previous is None or family.to_dict() == previous.to_dict():
            return
        if family.asset != previous.asset or family.timeframe != previous.timeframe:
            raise SnapshotVersionError("family identity cannot change partition")
        if family.version != previous.version + 1:
            raise SnapshotVersionError("family version must advance exactly one")
        if family.updated_at <= previous.updated_at:
            raise SnapshotVersionError("family updated_at must advance")

    def _validate_event_record(self, event: FamilyInteractionEvent) -> None:
        key = (event.asset, event.timeframe, event.event_id)
        existing = self._events.get(key, ())
        if not existing:
            return
        latest = existing[-1]
        if event.to_dict() == latest.to_dict():
            return
        if event.updated_at < latest.updated_at:
            raise SnapshotVersionError("event history cannot regress updated_at")
        if event.updated_at == latest.updated_at:
            raise SnapshotVersionError("event history cannot conflict at the same updated_at")

    def _record_family(self, family: TrendlineFamilyState) -> None:
        stored = self._clone_family(family)
        previous = self._families.get(stored.family_id)
        self._validate_family_record(stored)
        if previous is not None and stored.to_dict() == previous.to_dict():
            return
        self._families[stored.family_id] = stored
        self._family_history.setdefault(stored.family_id, []).append(stored)

    def _record_event(self, event: FamilyInteractionEvent) -> None:
        stored = self._clone_event(event)
        key = (stored.asset, stored.timeframe, stored.event_id)
        history = self._events.setdefault(key, [])
        self._validate_event_record(stored)
        if history and stored.to_dict() == history[-1].to_dict():
            return
        history.append(stored)

    def _events_at(self, *, asset: str, timeframe: str, as_of) -> tuple[FamilyInteractionEvent, ...]:
        events = []
        for (event_asset, event_timeframe, _), history in self._events.items():
            if event_asset != asset or event_timeframe != timeframe:
                continue
            event = next((item for item in reversed(history) if item.updated_at <= as_of), None)
            if event is not None:
                events.append(self._clone_event(event))
        return tuple(sorted(events, key=lambda event: (event.updated_at, event.event_id)))

    @staticmethod
    def _clone_family(family: TrendlineFamilyState) -> TrendlineFamilyState:
        return TrendlineFamilyState.from_dict(family.to_dict())

    @staticmethod
    def _clone_event(event: FamilyInteractionEvent) -> FamilyInteractionEvent:
        return FamilyInteractionEvent.from_dict(event.to_dict())
...
# Existing public implementation name remains valid after storage rehome.
InMemoryTrendlineFamilyRepository = InMemoryTrendlineRepository


__all__ = ["InMemoryTrendlineFamilyRepository", "InMemoryTrendlineRepository"]
