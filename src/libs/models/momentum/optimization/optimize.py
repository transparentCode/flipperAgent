"""CLI runner for Momentum optimization.

Usage:
    PYTHONPATH=src python -m libs.models.momentum.optimization.optimize \
        --asset BTCUSDT --timeframe 4h --n-trials 200 --audit --write-back
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Momentum optimization study"
    )
...
def main() -> None:
    ...
    objective_fn = mom_optimizer.make_objective(
        train_df, timeframe=args.timeframe, cost_bps=args.cost_bps,
    )

    n_trials = args.n_trials or mom_optimizer.STUDY_DEFAULTS.get("n_trials", 200)
    study_config = StudyConfig(
        model_name=mom_optimizer.MODEL_NAME,
        asset=args.asset,
        timeframe=args.timeframe,
        n_trials=n_trials,
        sampler=mom_optimizer.STUDY_DEFAULTS.get("sampler", "TPE"),
        pruner=mom_optimizer.STUDY_DEFAULTS.get("pruner", "MedianPruner"),
        objectives=["score"],
        directions=[mom_optimizer.STUDY_DEFAULTS.get("direction", "maximize")],
    )
    ...
    if args.write_back:
        write_best_params(
            mom_optimizer.MODEL_NAME, args.asset, args.timeframe, processed_params,
        )
