...
from apps.ingestion_app.jobs.gap_fill import run_rest_gap_fill
...
def patch_legacy_db_pool_manager(mock_asyncpg_pool, mocker):
    mocker.patch(
...
async def test_invalid_payload_rejection(base_worker_ctx, mock_ccxt_adapter, mock_asyncpg_pool):
    ...
    mock_df = pd.DataFrame(
        [[DEFAULT_MOCK_TIMESTAMP, 1000.0, 900.0, 950.0, 1000.0, 10.0]],
        columns=["timestamp", "open", "high", "low", "close", "volume"],
    )
