...
class TestOpenClose:
    ...
    def test_close_position_returns_pnl(self):
        ...
        tracker.open_position(pos)
        pnl = tracker.close_position("BTCUSDT", 0)
        assert pnl == 100
        assert tracker.get_position_count() == 0

    def test_close_invalid_index_raises(self):
        tracker = PositionTracker()
        with pytest.raises(IndexError):
            tracker.close_position("BTCUSDT", 0)
...
class TestExposure:
    def test_total_exposure(self):
        tracker = PositionTracker()
        tracker.open_position(_make_position(size=0.1, current_price=50_000))
    ...
    def test_all_positions(self):
        tracker = PositionTracker()
        tracker.open_position(_make_position())
        tracker.open_position(_make_position(asset="ETHUSDT"))
