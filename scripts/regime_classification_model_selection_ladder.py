#!/usr/bin/env python
"""Run simple-model selection benchmarks for RegimeClassification probabilities."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Simple MA/EMA model-selection ladder for regime probabilities.",
    )
    ...
    parser.add_argument("--event-quantiles", nargs="+", type=float, default=None)
    parser.add_argument("--n-bins-grid", nargs="+", type=int, default=None)
    parser.add_argument("--risk-budgets", nargs="+", type=float, default=None)
    parser.add_argument("--min-position-scales", nargs="+", type=float, default=None)
    parser.add_argument("--probability-policies", nargs="+", default=None)
    ...
    if args.event_quantiles is not None:
        prob_override["event_quantiles"] = args.event_quantiles
    if args.n_bins_grid is not None:
        prob_override["n_bins_grid"] = args.n_bins_grid
    if args.risk_budgets is not None:
        prob_override["risk_budgets"] = args.risk_budgets
    if args.min_position_scales is not None:
        prob_override["min_position_scales"] = args.min_position_scales
    if args.null_controls is not None:
        prob_override["null_controls"] = args.null_controls
    model_override: dict[str, Any] = {}
    if args.probability_policies is not None:
        model_override["probability_policies"] = args.probability_policies
    override: dict[str, Any] = {}
    if prob_override:
        override["probability_ladder"] = prob_override
    if model_override:
        override["model_selection_ladder"] = model_override
    settings = load_regime_optimization_settings(override or None)
    ...
    if args.input_csv:
        ...
        for asset in args.assets:
            for timeframe in args.timeframes:
                print(
                    f"running model-selection ladder asset={asset} timeframe={timeframe}",
                    file=sys.stderr,
                    flush=True,
                )
                reports.append(
    ...
    payload = {
        "reports": reports,
        "panel_summary": summarize_model_selection_panel(reports),
    }
...
def _run_one(
    ...
    *,
    ...
    if args.rolling:
        return run_rolling_model_selection_ladder(
            frame,
            asset=asset,
            timeframe=timeframe,
            settings=settings,
    ...
    return run_model_selection_ladder(
        frame,
        asset=asset,
        timeframe=timeframe,
        settings=settings,
