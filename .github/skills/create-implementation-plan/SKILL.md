...
This is a `quant-architect` capability.
Canonical skill: `.agents/skills/quant-architect/SKILL.md`.

Verify repository context and use memory only when prior decisions materially matter.
For existing code, inspect blast radius and affected execution flows.

Save plans under `plans/` only when persistence is requested or implementation will be
delegated. Include:
...
Do not leave placeholders or invent repository facts. The orchestrator owns durable
handoff persistence; return the contract in a form the orchestrator can save under
`plans/` when `quant-coder` is the next owner.
